...
Feature: Get Users from ReqRes API
  ...
  @regression @negative
  Scenario: Retrieve a user with invalid ID
    When I send a GET request to invalid id "https://reqres.in/api/users/9999"
    Then the get user with invalid id response status should be 404
    Then the response body should not contain user details

  @regression @negative
  Scenario: Create a user with missing fields
    When I send a POST request to "https://reqres.in/api/users" with missing API key and valid body:
      """
      {
        "name": ""
      }
    ...
    Then the create user error response status should be 401
